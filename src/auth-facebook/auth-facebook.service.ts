...
import { AuthFacebookLoginDto } from '~/auth-facebook/dto/auth-facebook-login.dto'
import { FacebookInterface } from '~/auth-facebook/interfaces/facebook.interface'
...
export class AuthFacebookService {
    ...
    private async verifyAccessToken(accessToken: string): Promise<void> {
        try {
            const appId = this.configService.get('facebook.appId', {
                infer: true,
            })
            ...
            if (!appId || !appSecret) {
                throw new HttpException('Facebook app credentials not configured', HttpStatus.INTERNAL_SERVER_ERROR)
            }
            ...
            if (tokenData.app_id !== appId) {
                throw new HttpException('Access token does not belong to this app', HttpStatus.UNAUTHORIZED)
            }
        } catch (error) {
            if (error instanceof HttpException) {
                throw error
